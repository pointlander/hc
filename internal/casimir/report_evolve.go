...
func (r EvolveReport) Markdown() string {
	...
	b.WriteString("The center sheet may **overhang the anodized plates**: only metal under the plates sees the 1 µm MIM gap; ")
	b.WriteString("overhang is still connected (higher inductance, no MIM C). Ranking fitness is ")
	b.WriteString("**⟨P₅₀⟩·A** (mean available thermal power into 50 Ω times total metal area). ")
	b.WriteString("Power is evaluated at 100 MHz, 433 MHz, 915 MHz, and 2.45 GHz. Sheet span is a gene; plate size is fixed. ")
	...
	fmt.Fprintf(&b, "| Plates (centered, square) | **%.1f mm** |\n", g.effectivePlateSpan()*1e3)
	fmt.Fprintf(&b, "| Sheet span range | %.1f–%.0f mm |\n\n", r.Config.MinSpan*1e3, r.Config.MaxSpan*1e3)

	b.WriteString("## Best sheet\n\n")
	fmt.Fprintf(&b, "| | |\n| --- | ---: |\n")
	fmt.Fprintf(&b, "| Sheet span | **%.2f mm** |\n", r.Best.Span*1e3)
	fmt.Fprintf(&b, "| Plates | %.2f mm |\n", g.effectivePlateSpan()*1e3)
	fmt.Fprintf(&b, "| Fill | **%.1f%%** (%d cells) |\n", 100*r.Best.Fill, g.nMetal())
	fmt.Fprintf(&b, "| Metal area | %.3f mm² |\n", g.Area()*1e6)
	fmt.Fprintf(&b, "| Overlap (under plates) | %.3f mm² |\n", g.OverlapArea()*1e6)
	fmt.Fprintf(&b, "| Overhang | %.3f mm² |\n", (g.Area()-g.OverlapArea())*1e6)
