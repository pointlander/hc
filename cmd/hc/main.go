...
func runSim(args []string) int {
	...
	width := fs.Float64("width", 100e-3, "E width along the spine, m")
	height := fs.Float64("height", 150e-3, "E height along the tines, m")
	spine := fs.Float64("spine", 20e-3, "spine bar thickness, m")
	arm := fs.Float64("arm", 20e-3, "tine width, m")
	thick := fs.Float64("thick", 0.4e-3, "E sheet thickness (gap direction), m")
	...
	fmt.Fprintf(os.Stderr, "E flat %.1f×%.1f mm (spine×tines)  oxide=%.3g µm  C=%.2f nF\n", d.Width*1e3, d.Height*1e3, d.Oxide*1e6, d.Capacitance()*1e9)
